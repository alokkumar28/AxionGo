      ...
      <main className="max-w-5xl mx-auto px-3 sm:px-5 lg:px-6 py-4 sm:py-5">
        ...
        <div className="space-y-4">
          {shopOrders.map((shopOrder) => {
            ...
            const deliveryBoyLocation = deliveryBoy?.location?.coordinates?.length === 2 ? { latitude: deliveryBoy.location.coordinates[1], longitude: deliveryBoy.location.coordinates[0] } : null;
              ...
              <section key={shopOrder._id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-3 sm:p-4 lg:p-5">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className={`shrink-0 px-2 sm:px-2.5 py-1 rounded-full text-[9px] sm:text-[11px] font-semibold whitespace-nowrap ${shopOrder.status === "Out for Delivery" ? "bg-purple-50 text-purple-700 border border-purple-200" : shopOrder.status === "Delivered" ? "bg-green-50 text-green-700 border border-green-200" : "bg-orange-50 text-orange-600 border border-orange-200"}`}>{shopOrder.status}</span>
                  </div>

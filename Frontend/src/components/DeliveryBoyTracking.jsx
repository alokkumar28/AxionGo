...
function DeliveryBoyTracking({ currentOrder }) {
  ...
  const isNearCustomer = () => {
    if (!deliveryBoyLocation?.latitude || !deliveryBoyLocation?.longitude || !customerLocation?.latitude || !customerLocation?.longitude) return false;

    const R = 6371;
    const dLat = ((customerLocation.latitude - deliveryBoyLocation.latitude) * Math.PI) / 180;
    const dLon = ((customerLocation.longitude - deliveryBoyLocation.longitude) * Math.PI) / 180;
          ...
          <div className="relative z-0 h-[280px] sm:h-[330px] lg:h-[380px] rounded-xl overflow-hidden border border-gray-200">
            {deliveryBoyPosition && customerPosition ? (
              <MapContainer center={deliveryBoyPosition} zoom={15} zoomControl={true} scrollWheelZoom={true} className="h-full w-full">
                ...
                <FitMapToLocations deliveryBoyLocation={deliveryBoyLocation} customerLocation={customerLocation} />

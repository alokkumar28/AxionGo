...
function SearchItemCard({ item }) {
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 group">
        ...
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-1">
            <div className="flex items-center gap-[1px]">
              {[1, 2, 3, 4, 5].map((star) =>
                star <= rating ? (
                  <FaStar key={star} className="text-yellow-400 text-[9px] sm:text-[10px]" />
                ) : (
                  <FaRegStar key={star} className="text-yellow-400 text-[9px] sm:text-[10px]" />
                )
              )}
            </div>
        ...
        <div className="flex items-center justify-between gap-2 mt-2.5">
          ...
          {quantity === 0 ? (
            ...
            <div className="flex items-center bg-orange-500 rounded-lg overflow-hidden text-white">
              <button onClick={() => { handleAddToCart(Math.max(quantity - 1, 0)); }} className="px-2 py-1.5 hover:bg-orange-600 transition">
                <FiMinus className="text-[11px]" />
              </button>
              <span className="min-w-[22px] text-center text-xs font-semibold">{quantity}</span>
              <button onClick={() => { handleAddToCart(quantity + 1); }} className="px-2 py-1.5 hover:bg-orange-600 transition">
